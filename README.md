...
If you are a new seller on AWS Marketplace, we advise you to read the AWS Marketplace - Seller Guide 
...
Once the resoveCustomer endpoint return successful response, the SaaS vendors must to provide access to the solution to the new subscriber. 
...
We have created MeteringSchedule CloudWatch Event rule that is **triggered every hour**. The `metering-hourly-job.js` gets triggered by this rule and it's quiring all of the pending/unreported metering records from the `AWSMarketplaceMeteringRecords` table using the PendingMeteringRecordsIndex.
...
After the record is submitted it will be updated and I.E. will look like this:
...
sam build
sam package --output-template-file packaged.yaml --s3-bucket <DEPLOYMENT_ARETFACTS_S3_BUCKET>
